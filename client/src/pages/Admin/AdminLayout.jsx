...
const SIDEBAR_LINKS = [
    ...
    { name: 'Quản lý danh mục', path: '/admin/categories', icon: ListTree },
    { name: 'Quản lý phim', path: '/admin/movies', icon: Film },
    { name: 'Quản lý rạp', path: '/admin/cinemas', icon: MapPin },
    { name: 'Quản lý suất chiếu', path: '/admin/showtimes', icon: Calendar },
    { name: 'Dịch vụ Bắp & Nước', path: '/admin/services', icon: Coffee },
    { name: 'Mã Giảm Giá', path: '/admin/vouchers', icon: Ticket },
    { name: 'Quà Tặng', path: '/admin/gifts', icon: Gift },
    { name: 'Quản lý Đặt vé', path: '/admin/bookings', icon: ReceiptText },
    { name: 'Quản lý Bình luận', path: '/admin/reviews', icon: MessageSquare },
        ...
        <div className="min-h-screen bg-[#050505] text-gray-200 flex overflow-hidden">
            ...
            >
                ...
                <div className="h-16 flex items-center px-4 border-b border-white/5 justify-between">
                    <Link to="/admin" className={`flex items-center gap-2 ${!sidebarOpen && 'lg:hidden'}`}>
                    ...
                    {SIDEBAR_LINKS.map((link) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`${!sidebarOpen && 'lg:hidden'} whitespace-nowrap`}>{link.name}</span>
                            </Link>
                        );
                    })}
                </nav>
                ...
                <div className="p-4 border-t border-white/5">
                    ...
                    >
                        <LogOut size={18} />
                        <span className={`${!sidebarOpen && 'lg:hidden'} whitespace-nowrap`}>Đăng xuất</span>
                    </button>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#050505]">
                {/* Top Header */}
                <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/5 z-30">
                    <div className="flex items-center gap-4">
                        <button className="text-gray-400 hover:text-white" onClick={() => setSidebarOpen(!sidebarOpen)}>
                            <Menu size={20} />
                        </button>
                        <h1 className="text-lg font-medium text-white hidden sm:block">
                            {SIDEBAR_LINKS.find((l) => l.path === location.pathname)?.name || 'Bảng điều khiển'}
                        </h1>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 pl-3 border-l border-white/10">
                            <div className="w-8 h-8 rounded-full bg-[#1a1a1a] border border-white/10 overflow-hidden flex items-center justify-center">
                                {dataUser?.avatar ? (
                                    <img src={dataUser.avatar} alt="Admin" className="w-full h-full object-cover" />
                                ) : (
